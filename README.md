...
Soal 3 adalah membangun infrastruktur server perpustakaan digital IT Library Nusantara menggunakan **Docker + Samba** dengan sistem permission yang berbeda untuk tiap role user.

Samba adalah protokol file sharing yang memungkinkan akses file lewat jaringan, mirip seperti Windows shared folder. Di sini Samba digunakan buat share koleksi perpustakaan ke anggota dengan hak akses yang berbeda-beda.

**Komponen yang dibuat:**

**`Dockerfile`:**
Build image dari `ubuntu:latest`, install Samba, buat struktur folder koleksi di `/libraryit/` (ebooks, papers, sourcecode, docs), copy konfigurasi `smb.conf` dan script `entrypoint.sh`, expose port 445 (SMB) dan 139 (NetBIOS).

**`entrypoint.sh`:**
Script yang dieksekusi otomatis tiap kali container pertama kali start. Tugasnya:
1. Buat group `readonly` dan `staff`
2. Buat user `member` (masuk group readonly), `contributor` (staff), `librarian` (staff)
3. Set password system pakai `chpasswd`
4. Daftarkan tiap user ke Samba password database pakai `smbpasswd`
5. Set permission folder sesuai aturan soal
6. Jalankan `smbd` di foreground

**`smb.conf`:**
File konfigurasi Samba yang mendefinisikan 4 share: `ebooks`, `papers`, `docs`, `sourcecode`. Tiap share punya `valid users` (siapa yang boleh akses) dan `write list` (siapa yang boleh nulis) yang berbeda.

**`docker-compose.yml`:**
Deploy 2 service sekaligus:
- `libraryit-server`: Samba server dengan bind mount semua folder koleksi dari host ke container
- `libraryit-logger`: Container ubuntu yang jalankan `tail -f` untuk monitor log Samba secara real-time

Port di-mapping ke `4455:445` dan `1399:139` karena port 445 default ga bisa dipakai di WSL (sudah dipakai Windows).

**Aturan akses:**
| User | ebooks | papers | sourcecode | docs |
|------|--------|--------|------------|------|
| member (readonly) | baca | baca | ❌ tidak bisa akses | baca |
| contributor (staff) | baca+tulis | baca+tulis | baca+tulis | baca |
| librarian (staff) | baca+tulis | baca+tulis | baca+tulis | baca+tulis |
...
[global]
   ...
   security = user        ; autentikasi pakai username/password
   map to guest = never   ; ga ada akses tanpa login
   log file = /logs/libraryit.log
   log level = 1          ; log akses normal dan yang ditolak
   max log size = 1000

; Share ebooks: bisa diakses readonly dan staff, tapi cuma staff yang bisa write
[ebooks]
...
; Share papers: sama seperti ebooks
[papers]
...
; Share docs: bisa diakses semua, tapi cuma librarian yang bisa write
; force create/directory mode diperlukan supaya permission filesystem
; sesuai dengan apa yang Samba harapkan
[docs]
   path = /libraryit/docs
   valid users = @readonly, @staff
   read only = yes
...
; Share sourcecode: hanya staff yang bisa akses (readonly tidak bisa lihat sama sekali)
[sourcecode]
...
# Buat group readonly dan staff
groupadd -f readonly
groupadd -f staff

# Buat user dengan opsi:
# -M: jangan buat home directory
# -s /sbin/nologin: user ini ga bisa login ke shell (cuma buat Samba)
# -G: masukkan ke group yang sesuai
...
# Set password sistem (diperlukan oleh Samba)
echo "member:member123" | chpasswd
echo "contributor:contrib456" | chpasswd
echo "librarian:lib789" | chpasswd

# Daftarkan ke Samba password database
# -a: tambah user baru, -s: silent mode (baca password dari stdin)
# Harus input password dua kali (konfirmasi), makanya pakai dua echo
...
# Enable user di Samba (aktifkan akun)
smbpasswd -e member
smbpasswd -e contributor
smbpasswd -e librarian

# Set permission folder:
# ebooks, papers, docs: 775 (owner+group bisa rw, others bisa r)
# sourcecode: 750 (owner+group bisa rw, others ga bisa akses)
chmod 775 /libraryit/ebooks
chmod 775 /libraryit/papers
chmod 750 /libraryit/sourcecode
chmod 775 /libraryit/docs

# Set ownership folder:
# ebooks, papers, sourcecode: milik root:staff
# docs: milik librarian:staff (supaya librarian bisa write)
...
# Jalankan smbd di foreground (bukan background)
# --no-process-group: jangan buat process group baru (diperlukan di Docker)
...
services:
  libraryit-server:
    ...
    ports:
      - "4455:445"              # port 445 default ga bisa dipakai di WSL
      - "1399:139"
    volumes:
      - ./data/ebooks:/libraryit/ebooks       # bind mount tiap koleksi
      - ./data/papers:/libraryit/papers       # dari host ke container
      - ./data/sourcecode:/libraryit/sourcecode
      - ./data/docs:/libraryit/docs
      - ./logs:/logs            # log bisa dipantau dari host
  ...
  libraryit-logger:
    ...
    depends_on:
      - libraryit-server        # tunggu server nyala dulu
    volumes:
      - ./logs:/logs            # akses file log yang sama
    command: tail -f /logs/libraryit.log   # monitor log real-time
    restart: unless-stopped
```
...
# Install smbclient untuk testing
sudo apt install smbclient -y

# Test member (readonly): bisa ls ebooks dan papers, TIDAK BISA akses sourcecode
smbclient //localhost/ebooks -p 4455 -U member%member123 -c "ls"
smbclient //localhost/papers -p 4455 -U member%member123 -c "ls"
smbclient //localhost/sourcecode -p 4455 -U member%member123 -c "ls"  # DENIED

# Test contributor (staff): bisa write di ebooks dan papers, TIDAK BISA write di docs
smbclient //localhost/ebooks -p 4455 -U contributor%contrib456 -c "put /etc/hostname test.txt"
smbclient //localhost/papers -p 4455 -U contributor%contrib456 -c "put /etc/hostname test.txt"
smbclient //localhost/docs -p 4455 -U contributor%contrib456 -c "put /etc/hostname test.txt"  # DENIED

# Test librarian: BISA write di docs
smbclient //localhost/docs -p 4455 -U librarian%lib789 -c "put /etc/hostname test.txt"

# Cek log akses
cat logs/libraryit.log

# Monitor log real-time via docker logs
...
**Screenshot 1 - Container berjalan**

`docker ps` nampilin kedua container `libraryit-server` dan `libraryit-logger` dengan status Up, plus container soal 2 yang juga masih aktif.

![SS1](assets/soal_3/ss1.png)

**Screenshot 2 - member bisa akses ebooks dan papers**

`smbclient //localhost/ebooks -p 4455 -U member%member123 -c "ls"` berhasil nampilin daftar file di share ebooks.

![SS2](assets/soal_3/ss2.png)

**Screenshot 3 - member TIDAK BISA akses sourcecode**

`smbclient //localhost/sourcecode -p 4455 -U member%member123 -c "ls"` ngembaliin `tree connect failed: NT_STATUS_ACCESS_DENIED`, membuktikan member sama sekali ga bisa akses sourcecode.

![SS3](assets/soal_3/ss3.png)

**Screenshot 4 - Verifikasi write permission**

contributor bisa write di ebooks dan papers (`putting file` berhasil), tapi DENIED di docs. librarian bisa write di docs (`putting file` berhasil).

![SS4](assets/soal_3/ss4.png)

**Screenshot 5 - Log akses tercatat**

`cat logs/libraryit.log` nampilin log akses termasuk baris `user 'member' not permitted to access this share (sourcecode)`, membuktikan logging berjalan dengan benar.
...
1. **`NT_STATUS_LOGON_FAILURE` saat login Samba** — `smbpasswd` tidak bisa nerima password lewat pipe `<<<`. Solusinya ganti ke format `(echo "password"; echo "password") | smbpasswd -a -s username` yang lebih reliable.

2. **Port 445 tidak bisa dipakai di WSL** — Port 445 sudah dipakai Windows untuk SMB. Solusinya mapping ke port lain: `4455:445` dan `1399:139`.

3. **librarian tidak bisa write di docs padahal permission filesystem sudah benar** — `smb.conf` butuh `force create mode` dan `force directory mode` supaya permission yang Samba set saat nulis file sesuai dengan permission folder di filesystem. Solusinya tambahkan `force create mode = 0664` dan `force directory mode = 0775` di section `[docs]`.

4. **contributor bisa write di docs padahal harusnya tidak boleh** — Konfigurasi lama pakai `write list = @staff` yang mencakup semua staff termasuk contributor. Solusinya ganti ke `write list = librarian` supaya hanya librarian yang bisa write.

5. **`version` attribute obsolete warning di docker compose** — Docker Compose versi baru udah ga butuh field `version` di `docker-compose.yml`. Warning ini muncul terus tapi tidak mengganggu jalannya program sama sekali.
